...
async function testUpload() {
  ...
  const postData = `file=${encodeURIComponent(file)}&upload_preset=${UPLOAD_PRESET}&public_id=${encodeURIComponent('test_auto_pdf_' + Date.now())}`;
  ...
  const req = https.request(options, (res) => {
    ...
    res.on('end', () => {
      ...
      if (res.statusCode === 200) {
        console.log("UPLOAD_SUCCESS_AUTO_BUCKET!");
        console.log("SECURE_URL:", data.secure_url);
        console.log("RESOURCE_TYPE:", data.resource_type);
      } else {
        console.error("UPLOAD_FAILED_AUTO_BUCKET:", body);
      }
    });
  });
