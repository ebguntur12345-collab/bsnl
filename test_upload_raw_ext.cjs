...
async function testUpload() {
  ...
  // IMPORTANT: public_id ends in .pdf and using raw bucket
  const postData = `file=${encodeURIComponent(file)}&upload_preset=${UPLOAD_PRESET}&public_id=${encodeURIComponent('test_raw_with_ext_' + Date.now() + '.pdf')}`;
  ...
  const req = https.request(options, (res) => {
    ...
    res.on('end', () => {
      ...
      if (res.statusCode === 200) {
        console.log("UPLOAD_SUCCESS_RAW_WITH_EXT!");
        console.log("SECURE_URL:", data.secure_url);
      } else {
        console.error("UPLOAD_FAILED_RAW_WITH_EXT:", body);
      }
    });
  });
