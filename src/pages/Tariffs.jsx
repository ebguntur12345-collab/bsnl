...
const TariffCard = ({ title, value, icon: Icon, colorClass, footerText = "Explore", onClick }) => (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
        {tariffData.map((item, index) => (
          <TariffCard 
            key={index} 
            {...item} 
            onClick={(categoryTitle) => navigate(`/module-documents/Tariffs/${encodeURIComponent(categoryTitle)}`)}
          />
        ))}
      </div>
    </div>
