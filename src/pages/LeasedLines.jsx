...
const LeasedLineCard = ({ title, value, icon: Icon, colorClass, footerText = "Explore" }) => (
  <div className="bg-white rounded shadow-sm overflow-hidden border border-gray-200 hover:shadow-md transition-shadow">
    ...
    <div className="bg-white px-4 py-2 border-t border-gray-50 flex items-center gap-2">
      ...
      <Link 
        to={`/documents/${title.replace(/\s+/g, '-')}`}
        className="text-[10px] font-bold text-blue-400 cursor-pointer hover:text-blue-600 transition-colors"
      >
        {footerText}
      </Link>
    </div>
