...
const BulkCCTCard = ({ title, value, icon: Icon, colorClass, footerText = "Explore", onClick }) => (
  <div className="bg-white rounded shadow-sm overflow-hidden border border-gray-200 hover:shadow-md transition-shadow">
    ...
    >
      ...
      <span className="text-[10px] font-bold text-blue-400 group-hover:text-blue-600 transition-colors">
        {footerText.replace('<', '') /* Remove the accidental typo in 'Explore' */}
      </span>
    </div>
  </div>
);
  ...
  return (
    <div className="p-4 space-y-6 animate-in fade-in duration-700 max-w-[1600px] mx-auto">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
        {bulkCCTData.map((item, index) => (
          <BulkCCTCard 
            key={index} 
            {...item} 
            onClick={(categoryTitle) => navigate(`/module-documents/Bulk%20CCTs/${encodeURIComponent(categoryTitle)}`)}
          />
        ))}
      </div>
    </div>
