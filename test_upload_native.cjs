...
async function testUpload() {
  ...
  const postData = `file=${encodeURIComponent(file)}&upload_preset=${UPLOAD_PRESET}&public_id=${encodeURIComponent('test_native_' + Date.now())}`;
  ...
  const req = https.request(options, (res) => {
    ...
    res.on('end', () => {
      ...
      if (res.statusCode === 200) {
        ...
        console.error("UPLOAD_FAILED:", body);
      }
    });
  });
